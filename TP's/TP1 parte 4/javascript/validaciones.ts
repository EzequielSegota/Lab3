...
function AdministrarValidacionesLogin()
{
    ...
    if(!(ret=ValidarCamposVacios("apellido"))){

        ret=true;
    }

    if(!(ret=ValidarCamposVacios("email"))){
        ret=true;
    }

    let dni:number= +(<HTMLInputElement> document.getElementById("txtDni")).value;
    if(!(ret=ValidarRangoNumerico(dni,1000000,50000000))){
        ret=true;
    }
}

function AdministrarSpanError(id:string,bool:boolean) {
    let campo=(<HTMLInputElement> document.getElementById(id)).value;
}
...
function ValidarCamposVacios(campo:string) :boolean  {
    ...
    switch (campo) {
        ...
        case "txtLegajo":
            ...
            break;
        case "email":
            let email:string=(<HTMLInputElement> document.getElementById(campo)).value;
            if(email.length==0)
            {
                alert("Email Vacio!");
                ret=false;
